...
Each service SHALL expose `/metrics` in Prometheus format. The standard set SHALL include `hive_mind_requests_total{tool,outcome}`, `hive_mind_stage_latency_seconds_bucket{stage}`, `hive_mind_tokens_total{stage,model,provider,tenant,direction}`, `hive_mind_provider_errors_total{provider,error_code}`, and ingestion counters per connector.
...
The system SHALL maintain a cost-per-1k-tokens table per `(provider, model, direction)` and SHALL compute and emit `hive_mind_cost_usd_total{provider,model,tenant}` alongside token counts.

#### Scenario: Cost computed for Anthropic call

- **WHEN** an Anthropic provider call records `tokens_in = 500`, `tokens_out = 100`
- **THEN** `hive_mind_cost_usd_total` increases by the configured rate for that model and direction
...
- **THEN** Grafana is provisioned with the shipped dashboards visible under a "Hive Mind" folder
