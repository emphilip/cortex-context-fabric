...
Every stage that invokes a model SHALL record `model`, `tokens_in`, `tokens_out`, `latency_ms`, and `provider` on its span and write a counter increment under `hive_mind_tokens_total{stage,model,provider,tenant}`.
