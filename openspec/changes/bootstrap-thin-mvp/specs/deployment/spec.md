...
A single `hive-mind.yaml` file SHALL configure the deployment. Environment variables MUST override file values using the convention `HIVE_MIND__<SECTION>__<KEY>` with double underscores between nesting levels.
...
- **WHEN** `HIVE_MIND__OLLAMA__BASE_URL` is set to `http://other-host:11434`
