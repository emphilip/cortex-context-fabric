...
- **WHEN** the operator runs `hive-mind-ingest git https://github.com/<owner>/<repo>` and the storage layer is healthy
- **THEN** new rows appear in `hive_mind.entity` with `source = "git"` and `source_revision` set to the cloned commit SHA
...
- **WHEN** the operator runs `hive-mind-ingest confluence ...` in the thin MVP
...
- **WHEN** the operator runs `hive-mind-ingest custom-api ...` in the thin MVP
- **THEN** the CLI exits non-zero with a message naming `not_implemented_in_mvp`
...
- **WHEN** the operator runs `hive-mind-ingest web ...` in the thin MVP
- **THEN** the CLI exits non-zero with a message naming `not_implemented_in_mvp`
