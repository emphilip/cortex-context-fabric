...
The system SHALL NOT consume a usage-feedback stream in v0. The MCP `hive_mind/submit_feedback` tool MUST return `not_implemented_in_mvp` when called.

#### Scenario: Feedback submission rejected in v0

- **WHEN** an MCP client calls `hive_mind/submit_feedback`
