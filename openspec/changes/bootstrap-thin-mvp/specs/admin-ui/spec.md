...
The admin UI SHALL NOT expose connector control in v0. Operators MUST trigger ingestion via the `hive-mind-ingest` CLI in v0; a follow-up change adds the UI surface.
...
- **WHEN** an operator runs `pnpm --filter @hive-mind/admin-ui storybook`
