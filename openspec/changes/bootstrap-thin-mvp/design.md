...
**Decision:** Keep `HIVE_MIND__OLLAMA__EMBEDDING_MODEL` env-overridable. Default to `nomic-embed-text` (768-dim, fast on CPU, the value already wired into Qdrant's collection vector size).

**Rationale:** users on bigger hardware can swap to `bge-m3` or `qwen3-embedding` by changing one env var and the Qdrant collection's vector size in `hive-mind.yaml`. Default stays small so the first-start model pull is bounded.
...
- **Risk:** Qdrant collection vector size (768) is hard-coded for `nomic-embed-text` in `hive-mind.yaml`. Swapping to a model with a different dimension breaks search until the collection is recreated. → Mitigation: keep `vector_size` in `hive-mind.yaml` adjacent to `embedding_model` so the link is obvious; admin UI for collection management is a follow-up.
...
2. Flip the default `HIVE_MIND__OLLAMA__BASE_URL` back to `http://ollama:11434` in `.env.example` and `hive-mind.yaml`. Move the Ollama Cloud example into commented documentation.
3. Update `.env` to point at the compose Ollama. Keep the Cloud key commented for future use.
4. Update `services/pipeline/src/hive_mind_pipeline/app.py` and the ingestion pipeline runner to handle both the legacy `/api/embeddings` and the newer `/api/embed` response shapes (we already accept both).
