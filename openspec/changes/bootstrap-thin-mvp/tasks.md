...
- [x] 1.6 `hive-mind.yaml` default configuration
...
- [x] 4.2 `tools/list` exposing five tool definitions namespaced `hive_mind/*`
- [x] 4.3 Identity stub + caller-supplied correlation-id preservation
- [x] 4.4 Pipeline client (undici) wired to `HIVE_MIND__PIPELINE__URL`
...
- [x] 7.1 AGE extension loaded + `hive_mind` graph created at DB init
- [ ] 7.2 Vocabulary, candidate-edge state machine, traversal — DEFERRED to follow-up change

## 8. Catalog store (already shipped)

- [x] 8.1 `hive_mind.entity` schema, stable UUID derivation
...
- [x] 10.1 `hive-mind-ingest` Click CLI
...
- [x] 12.3 Immutability trigger on `hive_mind.audit_log`
...
- [ ] 15.6 Pre-pull entrypoint or init script that runs `ollama pull $HIVE_MIND__OLLAMA__EMBEDDING_MODEL` on first start
- [ ] 15.7 `.env.example` and `hive-mind.yaml`: default `HIVE_MIND__OLLAMA__BASE_URL` to `http://ollama:11434`; move Cloud config into commented documentation
